...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (uploadForm) {
        uploadForm.addEventListener('submit', function (e) {
            ...
            // 2. Validate Case Number (Exactly 13 characters)
            const caseInput = document.getElementById('caseNumber');
            if (caseInput && caseInput.value.trim().length !== 13) {
                showError(caseInput, 'El número de caso debe tener exactamente 13 caracteres.');
                isValid = false;
            }
            ...
            if (phoneInput && phoneInput.value.trim()) {
                // Remove non-digits for checking content, but user might type dashes. 
                // User requirement: "10 numeros obligatoriamente ni mas ni menos"
                // Let's assume input cleaning or just length check on digits.
                const phoneDigits = phoneInput.value.replace(/\D/g, '');
                if (phoneDigits.length !== 10) {
                    showError(phoneInput, 'El número de teléfono debe tener 10 dígitos.');
                    isValid = false;
                }
            }
            ...
            if (emailInput && emailInput.value.trim()) {
                if (!emailInput.value.includes('@')) {
                    showError(emailInput, 'El correo electrónico debe contener un "@".');
                    isValid = false;
                } else {
                    // Keep regex for general validity if @ is present
            ...
            // 5. Validate Passport (Exactly 9 characters)
            const passportInput = document.getElementById('passportNumber');
            if (passportInput && passportInput.value.trim().length !== 9) {
                showError(passportInput, 'El número de pasaporte debe tener exactamente 9 caracteres.');
                isValid = false;
            }
